...
A frames2d pet may declare a `gameplay` block — an opt-in mini-game layer generalized from the miku desktop pet: decaying stat bars (`stats` with per-minute decay, a working variant and an idle variant), the unified treat (小鱼干) currency — gameplay income and shop spending ride the same panel treat stock (capped at 20): `work` success, `passiveIncome` and lottery prizes grant treats, and shop items are priced and paid in treats, with no separate wallet page — a weighted `idleDirector` (rolls an act every `intervalMs`, `maxMiss` forces one after consecutive idle rolls), `touch` zones inside a `hitBox` (roll branches with effects, a track hold and phrase bubbles), a `work` loop (host-adjudicated ticks with success/fail result tracks), a `sleep` loop (lazy stat restore), `modes` (extra named modes beyond work/sleep, each rendering its own menu button and carrying a label, an active label and an optional periodic stat restore), `roam` (a slow roll that walks the pet a random distance in one of four directions; the chrome owns the motion and lands the resting spot the way a drag does), `passiveIncome`, and a `shop` whose items carry effects or tiered lotteries. All rolls and bookkeeping are host-authoritative (`POST /api/pet/gameplay/*`); state persists per pet in `pet.json` and settles lazily on the treats-economy discipline. The browser half renders the menu card (stat bars, one button per declared mode, skin picker, shop grid) automatically for any pet that declares the block. The sprite display size range is 32–1024 px.
...
## Announcement bubbles (pet.announce)
...
The three most-liked pets in the Workshop's pet category on [dsh-market.com](https://dsh-market.com), in the site's default popularity order. Each is one `pet.json` plus its assets, rendered by the contracts above — no per-pet code:

| Pet | Renderer | Author | Source |
|---|---|---|---|
| [鲸鱼娘（精致版）](https://dsh-market.com/#pet:whale-girl-refined) (`whale-girl-refined`) | sprite2d | dsh-web | Built-in atlas `assets/whale-refined/` |
| [Miku](https://dsh-market.com/#pet:miku) (`miku`) | frames2d | stushansusu (MIT) | Workshop-only, installs into `$DSH_HOME/pets/miku/`; Hatsune Miku character rights belong to Crypton Future Media under the Piapro Character License |
| [女仆鲸鱼娘](https://dsh-market.com/#pet:jyn) (`jyn`) | frames2d | stushansusu (MIT) | Built-in `assets/jyn/` |

| 鲸鱼娘（精致版） | Miku | 女仆鲸鱼娘 |
|---|---|---|
| ![whale-girl-refined](assets/whale-refined/previews/idle.gif) | ![Miku](assets/miku/previews/idle.webp) | ![jyn](assets/jyn/previews/idle.webp) |

Sprite pets use 8-column atlases with 192×208 cells generated by the [hatch-pet](https://github.com/dsh2026) pipeline. Classic atlases carry 9 animation rows; v2 atlases add 2 rows for 16 look directions. 女仆鲸鱼娘 ships directory-style frame sequences instead, so its other gameplay states are separate preview files:

| shy | work | work success | sleep |
|---|---|---|---|
| ![jyn shy](assets/jyn/previews/shy.webp) | ![jyn work](assets/jyn/previews/work.webp) | ![jyn work-success](assets/jyn/previews/work-success.webp) | ![jyn sleep](assets/jyn/previews/sleep.webp) |
...
- **Selection & naming**: `petId` lives in the plugin's own configuration — the profile entry the Host serves the Pet settings page from; per-pet names live in `pet.json` under `names`, edited through the hover-panel rename of the active pet. Legacy installs migrate their flat `name` onto the whale girl.
- **Display**: `visible`, `size`, `right`, `bottom`, and `bubbleScale` persist in `pet.json` and are edited from the settings card. Bubble typography follows the sprite's own size (12px at the default 160px pet) and `bubbleScale` multiplies that result, bounded to 10–24px so a shrunk pet never carries unreadable text (issue #1549).
